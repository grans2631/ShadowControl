...
# Set CSV path
$CsvPath = "C:\Temp\ImageManager\Consolidatation Issues\ImageManager_Consolidation_Issues.csv"
...
try {
    ...
    foreach ($endpointKey in $Status.PSObject.Properties.Name) {
        ...
        $consolidationEntries = $entries | Where-Object { $_.Category -eq 'consolidation' }

        if ($consolidationEntries.Count -gt 0) {
            foreach ($entry in $consolidationEntries) {
    ...
    if ($csvRows.Count -eq 0) {
        Write-Output "No machines found with populated errors or warnings under consolidation."
        exit 0
    }
