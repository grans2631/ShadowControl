...
# Set CSV path
$CsvPath = "C:\Temp\ImageManager\Retention Issues\ImageManager_Retention_Issues.csv"
...
try {
    ...
    foreach ($endpointKey in $Status.PSObject.Properties.Name) {
        ...
        $retentionEntries = $entries | Where-Object { $_.Category -eq 'retention' }

        if ($retentionEntries.Count -gt 0) {
            foreach ($entry in $retentionEntries) {
    ...
    if ($csvRows.Count -eq 0) {
        Write-Output "No machines found with populated errors or warnings under retention."
        exit 0
    }
